...
from src.vad import AbstractTranscription, NonSpeechStrategy, PeriodicTranscriptionConfig, TranscriptionConfig, \
...
class VadOptions:
    def __init__(self, vad: str = None, vadMergeWindow: float = 5, vadMaxMergeSize: float = 150, vadPadding: float = 1,
                 vadPromptWindow: float = 1,
...
class WhisperTranscriber:
    ...
    def set_parallel_devices(self, vad_parallel_devices: str):
        self.parallel_device_list = [device.strip() for device in
                                     vad_parallel_devices.split(",")] if vad_parallel_devices else None
    ...
    def set_auto_parallel(self, auto_parallel: bool):
        if auto_parallel:
            ...
            print("[Auto parallel] Using GPU devices " + str(self.parallel_device_list) + " and " + str(
                self.vad_cpu_cores) + " CPU cores for VAD/transcription.")
    ...
    def transcribe_webui_simple(self, modelName, languageName, urlData, emailAddresses, multipleFiles, microphoneData,
                                task,
        ...
        return self.transcribe_webui_simple_progress(modelName, languageName, urlData, emailAddresses, multipleFiles,
                                                     microphoneData, task,
                                                     vad, vadMergeWindow, vadMaxMergeSize,
    ...
    def transcribe_webui_simple_progress(self, modelName, languageName, urlData, emailAddresses, multipleFiles,
                                         microphoneData, task,
                                         vad, vadMergeWindow, vadMaxMergeSize,
                                         word_timestamps: bool = False, highlight_words: bool = False,
                                         # diarization: bool = False, diarization_speakers: int = 2,
        ...
        vadOptions = VadOptions(vad, vadMergeWindow, vadMaxMergeSize, self.app_config.vad_padding,
                                self.app_config.vad_prompt_window, self.app_config.vad_initial_prompt_mode)
        ...
        return self.transcribe_webui(modelName, languageName, urlData, emailAddresses, multipleFiles, microphoneData,
                                     task, vadOptions,
                                     word_timestamps=word_timestamps, highlight_words=highlight_words,
                                     progress=progress)

    def transcribe_webui(self, modelName, languageName, urlData, emailAddresses, multipleFiles, microphoneData, task,
        ...
        try:
            ...
            if override_max_sources is not None and len(sources) > override_max_sources:
                raise ValueError("Maximum number of sources is " + str(override_max_sources) + ", but " + str(
                    len(sources)) + " were provided")
            ...
            try:
                ...
                for source in sources:
                    ...
                    if override_transcribe_file is None:
                        result = self.transcribe_file(model, source.source_path, selectedLanguage, task, vadOptions,
                                                      scaled_progress_listener, **decodeOptions)
                    else:
                    ...
                    ######################################################################
                    # Finish generating, save into DynamoDB
                    region_name = os.environ.get("REGION",
                                                 None)  # subprocess.getoutput('curl -s http://169.254.169.254/latest/meta-data/placement/region')
                    instance_id = os.environ.get("INSTANCE_ID",
                                                 None)  # subprocess.getoutput('curl -s http://169.254.169.254/latest/meta-data/instance-id')
                    ...
                    table_name = [tag['Value'] for tag in response['Tags'] if tag['Key'] == 'asr_content_table'][0]

                    dynamodb_cli = boto3.client('dynamodb', region_name=region_name)
                    dynamodb_table = boto3.resource('dynamodb', region_name=region_name).Table(table_name)

                    def insert_content(file_name, content):
                    ...
                    print("Pending to insert to DynamoDB...")
                    insert_content(source.source_path, source_text)
                    print("hh_meeting_minutes inserted")
                    ...
                    if (len(sources) > 1):
                        ...
                        zipFilePrefix = slugify(source_prefix + source.get_short_name(max_length=200),
                                                allow_unicode=True)
                ...
                if len(sources) > 1:
                    downloadAllPath = os.path.join(downloadDirectory,
                                                   "All_Output-" + datetime.now().strftime("%Y%m%d-%H%M%S") + ".zip")
    ...
    def transcribe_file(self, model: AbstractWhisperContainer, audio_path: str, language: str, task: str = None,
        ...
        if (vadOptions.vad == 'silero-vad'):
            ...
            period_config = PeriodicTranscriptionConfig(periodic_duration=vadOptions.vadMaxMergeSize,
    ...
    def process_vad(self, audio_path, whisperCallable, vadModel: AbstractTranscription, vadConfig: TranscriptionConfig,
        ...
        if (self.gpu_parallel_context is None):
            ...
            self.gpu_parallel_context = ParallelContext(num_processes=len(gpu_devices),
        ...
        if (self.cpu_parallel_context is None):
            self.cpu_parallel_context = ParallelContext(num_processes=self.vad_cpu_cores,
                                                        auto_cleanup_timeout_seconds=self.vad_process_timeout)

        parallel_vad = ParallelTranscription()
        return parallel_vad.transcribe_parallel(transcription=vadModel, audio=audio_path,
                                                whisperCallable=whisperCallable,
                                                config=vadConfig, cpu_device_count=self.vad_cpu_cores,
                                                gpu_devices=gpu_devices,
                                                cpu_parallel_context=self.cpu_parallel_context,
                                                gpu_parallel_context=self.gpu_parallel_context,
    ...
    def _create_silero_config(self, non_speech_strategy: NonSpeechStrategy, vadOptions: VadOptions):
        ...
        config = TranscriptionConfig(non_speech_strategy=non_speech_strategy,
                                     max_silent_period=vadOptions.vadMergeWindow,
                                     max_merge_size=vadOptions.vadMaxMergeSize,
                                     segment_padding_left=vadOptions.vadPadding,
                                     segment_padding_right=vadOptions.vadPadding,
                                     max_prompt_window=vadOptions.vadPromptWindow)
    ...
    def __get_subs(self, segments: Iterator[dict], format: str, maxLineWidth: int,
...
def create_ui(app_config: ApplicationConfig):
    ui = WhisperTranscriber(app_config.input_audio_max_duration, app_config.vad_process_timeout,
                            app_config.vad_cpu_cores,
    ...
    ui_description = "本demo实现了对于会议纪要音频的自动转录、自动总结及自动通知的功能。\n"
    ui_description += "1. 支持上传语音文件（.wav/.mp3) 以及使用microphone进行语音录入。音频文件会上传到后端服务器进行ASR(Automatic Speech Recognition)，并返回转录后的文字。\n"
    ui_description += "2. 支持输入收件人邮箱列表。当会议总结完后会自动发送邮件通知。\n"
    ui_description += "3. demo所涉及的模型、数据均不会离开您的亚马逊云账号，属于域内使用。"
    ...
    common_inputs = lambda: [
        ...
        gr.Text(label="URL (Website, etc.)"),
        gr.Text(label="Recipients Email Addresses(, as delimeter)"),
    ...
    common_vad_inputs = lambda: [
        gr.Dropdown(
            ...
            value=app_config.default_vad, label="VAD"),
    ...
    simple_transcribe = gr.Interface(
        fn=ui.transcribe_webui_simple_progress if is_queue_mode else ui.transcribe_webui_simple,
    ...
    demo = gr.TabbedInterface([simple_transcribe], tab_names=["Audio Meeting Assistant"])
    ...
    demo.launch(share=app_config.share,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--vad_initial_prompt_mode", type=str, default=default_app_config.vad_initial_prompt_mode,
                        choices=VAD_INITIAL_PROMPT_MODE_VALUES, \
    ...
    parser.add_argument("--whisper_implementation", type=str, default=default_whisper_implementation,
                        choices=["whisper", "faster-whisper"], \
                        help="the Whisper implementation to use")
    parser.add_argument("--compute_type", type=str, default=default_app_config.compute_type,
                        choices=["default", "auto", "int8", "int8_float16", "int16", "float16", "float32"], \
                        help="the compute type to use for inference")
    parser.add_argument("--threads", type=optional_int, default=0,
                        help="number of threads used by torch for CPU inference; supercedes MKL_NUM_THREADS/OMP_NUM_THREADS")

    parser.add_argument('--auth_token', type=str, default=default_app_config.auth_token,
