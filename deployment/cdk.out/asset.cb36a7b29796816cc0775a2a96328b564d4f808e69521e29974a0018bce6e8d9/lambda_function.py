...
def lambda_handler(event, context):
    ...
    try:
        ...
        if modelType == 'llama2':
            if task == 'chat':
                sys_prompt_template = get_prompt_template(language,modelType,task)
                chat_prompt_template = ''
            elif task == 'summarize':
                sys_prompt_template = get_prompt_template(language,modelType,'summarize-sys')
                chat_prompt_template = get_prompt_template(language,modelType,'summarize-chat') 
            print('sys_prompt_template:',sys_prompt_template)
            print('chat_prompt_template:',chat_prompt_template)
            
            result = chat_bot.get_chat_llama2(query,task,sys_prompt_template,chat_prompt_template,sessionId,TABLE_NAME)
        else:
            
            prompt_template = get_prompt_template(language,modelType,task)
            if "prompt" in evt_body.keys():
                prompt_template = evt_body['prompt']
            result = chat_bot.get_chat(query,task,prompt_template,sessionId,TABLE_NAME)
